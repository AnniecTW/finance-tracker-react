...
app.http("stats", {
  ...
  handler: async (request, context) => {
    try {
      function dashboardStats(allExpenses) {
        const now = new Date();
        ...
        const thisMonthBalance =
          totals.income.thisMonth - totals.expense.thisMonth;
        const lastMonthBalance =
          totals.income.lastMonth - totals.expense.lastMonth;
        const savingsRate =
      ...
      const result = dashboardStats(transactions ?? []);
